...
const runScript = (scriptNameOrOptions, maybeOptions = {}) => {
  const options =
    typeof scriptNameOrOptions === 'string'
      ? maybeOptions
      : scriptNameOrOptions !== null && typeof scriptNameOrOptions === 'object'
        ? scriptNameOrOptions
        : {};
...
describe('post-tool-use.js readConfigText with existing file', () => {
  it('uses memory-mason.json when no env vault path is set', () => {
    ...
    expect(fs.readFileSync(buildDailyFilePath(vaultPath, 'ai-knowledge', today()), 'utf-8')).toContain('output');
  });
});
