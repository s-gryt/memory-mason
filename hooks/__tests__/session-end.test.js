...
const runScript = (scriptName, options = {}) => {
  const stdinText = typeof options.stdinText === 'string' ? options.stdinText : (typeof options.payload !== 'undefined' ? JSON.stringify(options.payload) : '');
...
describe('session-end.js readConfigText with existing file', () => {
  it('uses memory-mason.json when no env vault path is set', () => {
    ...
    expect(fs.readFileSync(buildDailyFilePath(vaultPath, 'ai-knowledge', today()), 'utf-8')).toContain('cfg-test /');
  });
});
