...
describe('post-tool-use.js readStdin', () => {
  it('returns valid JSON string from mocked fd 0', () => {
    ...
    const buf = Buffer.from(payload);
    let rc = 0;
    expect(postToolUse.readStdin({ readSync(fd, chunk) { if (rc === 0) { rc++; buf.copy(chunk); return buf.length; } return 0; } })).toBe(payload);
  });
...
describe('post-tool-use.js serializeToolResponse', () => {
  ...
  it('joins text blocks for array with valid text blocks', () => {
    expect(postToolUse.serializeToolResponse([{ type: 'text', text: 'a' }, { type: 'text', text: 'b' }])).toBe('a\nb');
  });
  ...
  it('falls back to JSON for array with empty text blocks', () => {
    const arr = [{ type: 'text', text: '   ' }, { type: 'text', text: '' }];
    expect(postToolUse.serializeToolResponse(arr)).toBe(JSON.stringify(arr, null, 2));
  });

  it('filters out null, non-object, non-text-type, non-string-text elements', () => {
    const arr = [null, 'str', { type: 'image', text: 'no' }, { type: 'text', text: null }, { type: 'text', text: 'yes' }];
...
describe('post-tool-use.js runtime fallback branches', () => {
  it('resolveRuntimeEnv falls back when env is null', () => {
    const result = postToolUse.run(JSON.stringify({ hook_event_name: 'PostToolUse', cwd: createTempDir('mm-cwd-'), tool_name: 'Bash', tool_response: 'x' }), { env: null, cwd: createTempDir('mm-cwd-'), homedir: createTempDir('mm-h-') });
    expect(result.status).toBe(0);
  });

  it('resolveFallbackCwd falls back when cwd is not a string', () => {
    const homeDir = createTempDir('mm-home-');
    const result = postToolUse.run(JSON.stringify({ hook_event_name: 'PostToolUse', cwd: homeDir, tool_name: 'Bash', tool_response: 'x' }), { env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: createTempDir('mm-v-') }), cwd: 123, homedir: homeDir });
    expect(result.status).toBe(0);
  });

  it('resolveRuntimeHomedir falls back when homedir is not a string', () => {
    const homeDir = createTempDir('mm-home-');
    const result = postToolUse.run(JSON.stringify({ hook_event_name: 'PostToolUse', cwd: homeDir, tool_name: 'Bash', tool_response: 'x' }), { env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: createTempDir('mm-v-') }), cwd: homeDir, homedir: 42 });
    expect(result.status).toBe(0);
  });
});
...
describe('post-tool-use.js main', () => {
  it('calls exit 0 after writing tool output', () => {
    const homeDir = createTempDir('mm-home-');
    const vaultPath = createTempDir('mm-vault-');
    const payload = JSON.stringify({ hook_event_name: 'PostToolUse', cwd: homeDir, tool_name: 'Bash', tool_response: 'main out' });
    const buf = Buffer.from(payload);
    let rc = 0;
    const writes = []; const errors = []; let exitCode = null;
    postToolUse.main({
  ...
  it('writes stderr on config failure', () => {
    const homeDir = createTempDir('mm-home-');
    const payload = JSON.stringify({ hook_event_name: 'PostToolUse', cwd: createTempDir('mm-nocfg-'), tool_name: 'Bash', tool_response: 'x' });
    const buf = Buffer.from(payload);
    let rc = 0;
    const errors = []; let exitCode = null;
    postToolUse.main({
  ...
  it('falls back to process stdout/stderr when io functions are missing', () => {
    const homeDir = createTempDir('mm-home-');
    const vaultPath = createTempDir('mm-vault-');
    const payload = JSON.stringify({ hook_event_name: 'PostToolUse', cwd: homeDir, tool_name: 'Read', tool_response: '' });
    const buf = Buffer.from(payload);
    let rc = 0;
...
describe('pre-compact.js readStdin', () => {
  it('reads single chunk from mocked fd 0', () => {
    ...
    const buf = Buffer.from(payload);
    let rc = 0;
    expect(preCompact.readStdin({ readSync(fd, chunk) { if (rc === 0) { rc++; buf.copy(chunk); return buf.length; } return 0; } })).toBe(payload);
  });
...
describe('pre-compact.js main', () => {
  it('reads stdin via mock fs and calls exit', () => {
    ...
    writeText(transcriptPath, buildTranscript(6));
    const payload = JSON.stringify({ cwd: hooksRoot, transcript_path: transcriptPath, session_id: 'session-main' });
    const buf = Buffer.from(payload);
    let rc = 0;
    const errors = []; let exitCode = null;
    preCompact.main({
      fs: { readSync(fd, chunk) { if (rc === 0) { rc++; buf.copy(chunk); return buf.length; } return 0; } },
      cwd: hooksRoot, env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }), homedir: homeDir,
      io: { stdout: () => {}, stderr: (t) => errors.push(t), exit: (c) => { exitCode = c; } }
    });
    expect(exitCode).toBe(0);
    expect(errors).toHaveLength(0);
  ...
  it('writes stderr on error', () => {
    ...
    preCompact.main({
      fs: { readSync(fd, chunk) { if (rc === 0) { rc++; buf.copy(chunk); return buf.length; } return 0; } },
      cwd: hooksRoot, env: buildEnv(createTempDir('mm-h-')),
      io: { stdout: () => {}, stderr: (t) => errors.push(t), exit: (c) => { exitCode = c; } }
    });
    expect(exitCode).toBe(0);
...
describe('session-end.js readStdin', () => {
  it('reads single chunk from mocked fd 0', () => {
    ...
    const buf = Buffer.from(payload);
    let rc = 0;
    expect(sessionEnd.readStdin({ readSync(fd, chunk) { if (rc === 0) { rc++; buf.copy(chunk); return buf.length; } return 0; } })).toBe(payload);
  });
...
describe('session-end.js utility functions', () => {
  ...
  it('findCopilotCliSessionContent throws on missing session-state dir', () => {
    expect(() => sessionEnd.findCopilotCliSessionContent(path.join(createTempDir('mm-m-'), 'no'), '')).toThrow('copilot session-state dir not found');
  });

  it('findCopilotCliSessionContentOrEmpty returns empty on missing dir', () => {
    expect(sessionEnd.findCopilotCliSessionContentOrEmpty(path.join(createTempDir('mm-m-'), 'no'), '')).toBe('');
  });
  ...
  it('collectAssistantTurnContents skips null and empty entries', () => {
    expect(sessionEnd.collectAssistantTurnContents([null, { role: 'assistant', content: '' }, { role: 'assistant', content: 'ok' }], 0)).toEqual(['ok']);
  });

  it('getLastAssistantTurnContent returns last assistant or empty', () => {
    expect(sessionEnd.getLastAssistantTurnContent([{ role: 'assistant', content: 'a' }, { role: 'assistant', content: 'b' }])).toBe('b');
...
describe('session-end.js stop with empty session ID', () => {
  it('returns early', () => {
    const homeDir = createTempDir('mm-home-');
    const vaultPath = createTempDir('mm-vault-');
    const result = sessionEnd.run(JSON.stringify({ hook_event_name: 'Stop', cwd: hooksRoot, transcript_path: path.join(createTempDir('mm-tr-'), 'x.jsonl') }), {
      cwd: hooksRoot, env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }), homedir: homeDir
    });
...
describe('session-end.js transcript with 0 turns on non-stop event', () => {
  it('skips when transcript has no user/assistant turns', () => {
    ...
    const result = runScript('session-end.js', {
      payload: { hook_event_name: 'session_end', cwd: hooksRoot, transcript_path: transcriptPath, session_id: 'session-zero' },
      env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath })
    });
    expect(result.status).toBe(0);
  });
...
describe('session-end.js main', () => {
  it('reads stdin via mock fs and calls exit', () => {
    ...
    const transcriptPath = path.join(createTempDir('mm-tr-'), 'session.jsonl');
    writeText(transcriptPath, buildTranscript(2));
    const payload = JSON.stringify({ hook_event_name: 'session_end', cwd: hooksRoot, transcript_path: transcriptPath, session_id: 'se-main' });
    const buf = Buffer.from(payload);
    let rc = 0;
    const errors = []; let exitCode = null;
    sessionEnd.main({
...
describe('pre-compact.js runtime fallback branches', () => {
  it('falls back to process defaults when runtime properties are invalid', () => {
    const result = preCompact.run(JSON.stringify({ cwd: createTempDir('mm-cwd-') }), { env: null, cwd: 123, homedir: 42 });
    expect(result.status).toBe(0);
  });
  ...
  it('uses fallbackCwd when input has no cwd', () => {
    ...
    const transcriptPath = path.join(createTempDir('mm-tr-'), 'session.jsonl');
    writeText(transcriptPath, buildTranscript(6));
    const result = preCompact.run(JSON.stringify({ transcript_path: transcriptPath, session_id: 'nocwd' }), {
      env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }),
      cwd: createTempDir('mm-fb-'), homedir: homeDir
    });
    expect(result.status).toBe(0);
  });
});
...
describe('session-end.js runtime fallback branches', () => {
  it('falls back to process defaults when runtime properties are invalid', () => {
    const result = sessionEnd.run(JSON.stringify({ hook_event_name: 'session_end', cwd: createTempDir('mm-cwd-') }), { env: null, cwd: 123, homedir: 42 });
    expect(result.status).toBe(0);
  });
  ...
  it('uses fallbackCwd when input has no cwd', () => {
    ...
    const result = sessionEnd.run(
      JSON.stringify({ hook_event_name: 'session_end' }),
      { env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }), cwd: createTempDir('mm-fb-'), homedir: homeDir }
    );
    expect(result.status).toBe(0);
  });
...
describe('post-tool-use.js extractToolPayload unsupported platform', () => {
  it('throws for unsupported platform', () => {
    expect(() => postToolUse.extractToolPayload('unknown-platform', {})).toThrow('unsupported platform: unknown-platform');
  });
});
...
describe('session-end.js findCopilotCliSessionContent no jsonl files', () => {
  it('throws when dirs exist but contain no .jsonl files', () => {
    ...
    expect(() => sessionEnd.findCopilotCliSessionContent(sessionStateDir, '')).toThrow('no .jsonl files found in copilot session-state');
  });
});
...
describe('post-tool-use.js copilot-cli payload with null toolResult', () => {
  it('falls back to empty object when toolResult is null', () => {
    ...
    const result = postToolUse.run(
      ...
      { env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }), cwd: homeDir, homedir: homeDir }
    );
    expect(result.status).toBe(0);
  });
});
...
describe('post-tool-use.js textResultForLlm branch', () => {
  it('handles copilot-vscode payload with textResultForLlm', () => {
    ...
    const vaultPath = createTempDir('mm-vault-');
    const result = postToolUse.run(
      JSON.stringify({ hookEventName: 'PostToolUse', cwd: homeDir, toolName: 'Bash', toolResult: { textResultForLlm: 'llm text' } }),
      { env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }), cwd: homeDir, homedir: homeDir }
    );
    expect(result.status).toBe(0);
  });
  ...
  it('handles copilot-vscode payload without textResultForLlm', () => {
    ...
    const vaultPath = createTempDir('mm-vault-');
    const result = postToolUse.run(
      JSON.stringify({ hookEventName: 'PostToolUse', cwd: homeDir, toolName: 'Bash', toolResult: { something: 'else' } }),
      { env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }), cwd: homeDir, homedir: homeDir }
    );
    expect(result.status).toBe(0);
  });
...
describe('post-tool-use.js MEMORY_MASON_INVOKED_BY skip', () => {
  it('returns empty when MEMORY_MASON_INVOKED_BY is set', () => {
    const homeDir = createTempDir('mm-home-');
    const result = postToolUse.run(
      JSON.stringify({ hook_event_name: 'PostToolUse', cwd: createTempDir('mm-cwd-'), tool_name: 'Bash', tool_response: 'x' }),
...
describe('session-end.js readConfigText with existing file', () => {
  it('uses memory-mason.json when no env vault path is set', () => {
    ...
    writeText(path.join(cwd, 'memory-mason.json'), JSON.stringify({ vaultPath, subfolder: 'ai-knowledge' }));
    const result = runScript('session-end.js', {
      payload: { hook_event_name: 'session_end', cwd, transcript_path: transcriptPath, session_id: 'cfg-test' },
      cwd, env: buildEnv(homeDir)
    });
    expect(result.status).toBe(0);
  });
...
describe('session-end.js findCopilotCliSessionContent with matching cwd', () => {
  it('returns content when matching cwd found in jsonl', () => {
    ...
    writeText(path.join(subDir, 'session.jsonl'), JSON.stringify({ message: { role: 'user', content: 'hello from ' + targetCwd } }));
  ...
  it('falls back to first dir when cwd not matched', () => {
    const sessionStateDir = createTempDir('mm-copilot-');
    const subDir = path.join(sessionStateDir, 'session-a');
    writeText(path.join(subDir, 'session.jsonl'), JSON.stringify({ message: { role: 'user', content: 'hello' } }));
...
describe('session-end.js calculateNextTurnCount edge cases', () => {
  it('handles stop event with non-empty last_assistant_message and empty transcript', () => {
    ...
    const vaultPath = createTempDir('mm-vault-');
    const result = sessionEnd.run(
      JSON.stringify({ hook_event_name: 'Stop', cwd: hooksRoot, session_id: 'calc-test', last_assistant_message: 'test reply' }),
      { cwd: hooksRoot, env: buildEnv(homeDir, { MEMORY_MASON_VAULT_PATH: vaultPath }), homedir: homeDir }
    );
    expect(result.status).toBe(0);
  });
