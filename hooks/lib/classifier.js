...
const { assertObjectRecord } = require("./assert");
const {
  USER_INPUT_TOOLS,
  NOISY_TOOLS,
  META_TOOLS,
...
/**
 * @typedef {Object} ToolEventClassificationInput
...
/**
 * Classify a PostToolUse event into one of the EVENT_TYPE_* constants.
 * Returns the first matching classification in precedence order.
 *
 * @param {ToolEventClassificationInput} input
 * @returns {string}
 */
function classifyToolEvent(input) {
  ...
  if (hasAnyPattern(output, TEST_RESULT_PATTERNS)) {
    return EVENT_TYPE_TEST_RESULT;
  }

  if ((exitCode !== NULL_VALUE && exitCode !== ZERO) || hasAnyPattern(output, ERROR_PATTERNS)) {
    return EVENT_TYPE_ERROR;
  }

  if (isNoiseContent(output)) {
    return EVENT_TYPE_NOISE;
  }

  if (PLAN_OUTPUT_TOOLS.has(toolName) && normalizedFilePath.includes(PLAN_PATH_PATTERN)) {
    return EVENT_TYPE_PLAN_OUTPUT;
  }

  if (AGENT_TOOLS.has(toolName)) {
    return EVENT_TYPE_AGENT_RESULT;
  }

  if (WRITE_TOOLS.has(toolName) && lineCount > DISCOVERY_MIN_LINES) {
    return EVENT_TYPE_DISCOVERY;
  }
  ...
  if (NOISY_TOOLS.has(toolName) || isBashExploration(toolName, commandText)) {
    return EVENT_TYPE_EXPLORATION;
  }
